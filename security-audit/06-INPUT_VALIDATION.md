...
While the path is fixed (`~/.scriptkit/config.ts`), a malicious symlink or modified config.ts could execute arbitrary code.
...
2. Config files in `~/.scriptkit/`
3. Theme files in `~/.scriptkit/`
4. Scriptlet files
