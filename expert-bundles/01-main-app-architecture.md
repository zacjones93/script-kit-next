...
────────────────
  ...
  Total Tokens: ~20.8K (20,782 exact)
  Total Chars: 117,678 chars
       Output: -
...
This is `main.rs` - the **application entry point** for Script Kit GPUI, a launcher/automation tool built with Zed's GPUI framework. This file handles app initialization, window creation, global hotkeys, file watchers, and the main event loop.

## What This File Does

1. **App Initialization** - GPUI app setup, font loading, theme initialization
2. **Window Creation** - Main window with gpui-component Root wrapper
3. **Global Hotkeys** - Main window, Notes, AI window shortcuts
4. **File Watchers** - Theme, config, scripts, appearance changes
5. **Script Scheduler** - Cron-based script scheduling
6. **MCP Server** - Model Context Protocol server startup
7. **Stdin Listener** - JSON commands for automation/testing
8. **Signal Handlers** - Graceful shutdown, orphan process cleanup

## What We Need Reviewed

### 1. Initialization Order
We initialize in this order: fonts → theme → config → watchers → hotkeys → window

**Questions:**
- Is this order correct for all edge cases?
- What if theme loading fails?
- Should we have a "degraded mode" fallback?

### 2. File Watcher Setup
Multiple watchers for different concerns:
- Theme changes (`~/.scriptkit/theme.json`)
- Config changes (`~/.scriptkit/config.ts`)
- Script changes (`~/.scriptkit/scripts/`)
- macOS appearance changes

**Questions:**
- Are we handling watcher errors correctly?
- Is debouncing (500ms) appropriate?
- Should watchers be lazy-initialized?

### 3. Global Hotkey Registration
We register hotkeys for main window, Notes, AI:
```rust
register_global_hotkey(&config.hotkey, ...);
```

**Questions:**
- How do we handle registration failures?
- What if the hotkey conflicts with another app?
- Should we retry registration?

### 4. Graceful Shutdown
On exit, we:
- Kill orphaned script processes
- Close file watchers
- Unregister hotkeys

**Questions:**
- Are we cleaning up everything?
- What about async operations in flight?
- How do we handle forced termination?

### 5. Error Handling at Startup
Currently we use `expect()` for critical failures.

**Questions:**
- Should we show a GUI error instead of panic?
- What errors are recoverable?
- Should we log startup errors to a file?

## Specific Code Areas of Concern

1. **`cx.spawn()` detached tasks** - Are we tracking them for cleanup?
2. **`OnceLock` for global state** - Is this the right pattern?
3. **Thread spawning** - stdin listener, file watchers
4. **Process cleanup** - Walking `/proc` or using `pgrep`

## Deliverables Requested

1. **Initialization audit** - Order and error handling
2. **Resource cleanup review** - Are we leaking anything?
3. **Error strategy** - How to handle startup failures gracefully
4. **Thread management** - Are spawned threads properly tracked?
5. **Simplification opportunities** - Can we reduce complexity?
